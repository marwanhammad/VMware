...
$AdminPortGroup = Get-VirtualPortGroup | where{$_.Name -like "*$vDSID*$AdminLW_Name*" }
...
$InetPortGroup = Get-VirtualPortGroup | where{$_.Name -like "*$vDSID*$InetLW_Name*" }
...
Foreach ( $OrgVDC in $OrgVDCsList  ){
	...
	if ($OrgVDCURL.count -eq '1'){
		...
		Try{
			...
			Check-vCD-Task $TaskURL
			#$OrgVDCAdminURL = $responseOrgVDC.AdminOrg.href
		}catch{
			Write-Warning ("Invoke-vCloud Exception: $($_.Exception.Message)")
			if ( $_.Exception.ItemName ) { Write-Warning ("Failed Item: $($_.Exception.ItemName)") }
			#Exit-function
