...
$LW_Name = $Customer.customer.FullName + $Customizations.AdminPESG.LSWName 
...
########################## Edit XML file for External network Creation #######################
Write-Host (Get-Date).ToString() "$ScriptIdentifier : Updating XML file" -ForegroundColor Green
