			...
			$EdgesList= $OrgVDC.Edges.Edge
				Foreach ( $Edge in $EdgesList  ){
					...
					Try{
							...
							$TaskStatus = Check-vCD-Task $TaskURL
							####https://3.flexible-computing-advanced.orange-business.com/network/edges/5d32f31e-f4b4-4c9d-ab37-799b465019a2
							
							
							Write-Host (Get-Date).ToString() "$ScriptIdentifier : Enable OSPF on Edge :$OrgVDCName" -ForegroundColor Green
							############################# Enable Edge OSPF ###############################
							########################### Get Edge ID ######################################
							$responseEdgeHref = $responseEdge.EdgeGateway.href
							$separator = "/"
							$option = [System.StringSplitOptions]::RemoveEmptyEntries
							[array] $OrgvDCEdge = $responseEdgeHref.Split($separator,$option)
		                    $VSEID = $OrgvDCEdge[5]
	
							#$VSEID="5d32f31e-f4b4-4c9d-ab37-799b465019a2"
							
							##################### GET Edge Primary Address ###############################
							$GetEdgeURL=  $VCDURL +"/network/edges/$VSEID"
							$responseOrgVDCEdge=Invoke-RestMethod -Uri $GetEdgeURL -Headers $headers -Method GET -WebSession $MYSESSION
                            $PrimaryAddress = $responseOrgVDCEdge.edge.vnics.vnic[0].addressGroups.addressGroup.primaryAddress
							$XMLLocation = $WorkingDirectory+'Templates\VSEEdgeOSPF.xml'
					        Write-Host (Get-Date).ToString() "$ScriptIdentifier : Import XML file $XMLLocation" -ForegroundColor Green
						    $xmltemplateVCDEdgeOSPF = (Get-Content $XMLLocation  -ErrorVariable ProcessError -ErrorAction SilentlyContinue)
							
							$xmlVSEOSPF = $xmltemplateVCDEdgeOSPF 
							$xmlVSEOSPF = $xmlVSEOSPF -replace "VSERouterID", $PrimaryAddress
							$GetEdgeOSPFURL=  $VCDURL +"/network/edges/$VSEID/routing/config"
							
  							$ContentType = "application/xml"
							$responseEdgeOspf = Invoke-RestMethod   -Uri $GetEdgeOSPFURL  -Headers $headers  -Method PUT -Body $xmlVSEOSPF -ContentType $ContentType
					        Invoke-RestMethod   -Uri $GetEdgeOSPFURL  -Headers $headers  -Method PUT -Body $xmlVSEOSPF -ContentType $ContentType
							Write-Host (Get-Date).ToString() "$ScriptIdentifier : Task submitted successfully " -ForegroundColor Green
